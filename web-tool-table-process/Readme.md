...
* This process assumes that cancer incidence rates for zones and the state as a whole have been calculated using the Zone Rate Calculations process.  Copy the two resulting SEERStat matrix files (`[RunNum].zone_RateCalcs.txt` and `[StAbbr].state_RateCalcs.txt`) to the WebToolTables folder. 
...
* Run the 51_ZoneWebToolRates SAS program to create the cancer incidence rate table for the web tool based on the SEERStat results
	* Set the %let parameters at the beginning of the code to specify:
		...
		* Whether or not to include national rates and, if so, the name of the SEERStat matrix output dataset with the appropriate national cancer rates.  National rates are available based on USCS data in the `National_Cancer_Rates` folder.  Use the dataset with the ending year you need.  For example, `AllUS_Combined_v3_to2017.txt` has 1-year, 5-year, and 10-year data ending in 2017.
	...
	* In addition, the program generates a summary report and Excel file showing how many cells are suppressed for the different by-levels and for 1, 5, and 10 years of data.  This summary can be useful to evaluate and perhaps modify decisions about which cancer sites, race/ethnicities, and year groupings to include in the tool.
...
* Run the 52_ZoneWebToolDemog SAS program to create the demographic data table for the web tool
	* Set the %let parameters at the beginning of the code to specify:
		* The state, state abbreviation, and final run number
		* Whether or not to include national data
		* Demographic variables to include from Census 2010 and ACS.  For ACS variables, there is an option to include two data periods to calculate the demographic statistic across 10 years to match the 10-year cancer data period. For the ACS parameters, set:
			* Whether to use one ACS period or two
			* Years associated with the ACS period(s) (the end year of the 5-year ACS period)
			* What ACS variables to include (keepACS for period 1, keepACS2 for period 2).  Normally these will be the same unless a particular variable is not available in both periods.  
		* The data path if different from `C:\Work\WebToolTables`
	* The program currently assumes that any needed Census 2010 variables are included in the ZoneList dataset.  ACS variables can be drawn from Westat’s tract-level ACS data extract (copies and data dictionaries are available in the `Census_Data_Tables` folder).
	* Investigate any warning messages generated by the program as these usually mean something is not working as expected
